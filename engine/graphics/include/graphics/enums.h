...
typedef enum DebugSource
{
    DebugSourceAPI            = 0, // Messages generated directly by explicit graphics API calls.
    DebugSourceWindowSystem   = 1, // Messages originating from the underlying window-system graphics API.
    DebugSourceShaderCompiler = 2, // Messages produced by the shader compiler or linker.
    DebugSourceThirdParty     = 3, // Messages generated by third-party libraries or middleware utilizing the graphics API.
    DebugSourceApplication    = 4, // Custom messages explicitly injected by your own application code.
    DebugSourceOther          = 5, // Messages from sources that do not fit into any other category.
    DebugSourceCount,
} DebugSource;
...
{
    DebugTypeError               = 0, // High-priority events indicating that an actual, invalid graphics API error has occurred.
    DebugTypeDeprecatedBehaviour = 1, // Warnings that a used feature has been marked as deprecated in the current context.
    DebugTypeUndefinedBehaviour  = 2, // Critical warnings that an operation triggered behavior not explicitly defined by the specification.
    DebugTypePortability         = 3, // Messages highlighting usage that may behave differently across various vendors or platforms.
    DebugTypePerformance         = 4, // Insights indicating that the current usage is suboptimal for the graphics hardware.
    DebugTypeMarker              = 5, // Annotations marking specific command stream milestones (often used in external tools like RenderDoc).
    DebugTypePushGroup           = 6, // Events indicating the push/entry into a nested debug group scope.
    DebugTypePopGroup            = 7, // Events indicating the pop/exit from the current nested debug group scope.
    DebugTypeOther               = 8, // Messages representing events that do not fall under any standard type category.
    DebugTypeCount,
} DebugType;

uint32_t debugTypeToNative(DebugType debugType);

typedef enum DebugSeverity
{
    DebugSeverityVerbose = 0, // Used for low-priority, purely informational messages or vendor-specific hints that have no performance or functional impact.
    DebugSeverityInfo    = 1, // Used for minor, non-critical issues like compilation log details or small, non-urgent performance recommendations.
    DebugSeverityWarning = 2, // Used for moderate issues such as severe performance bottlenecks, usage of deprecated behaviors, or platform-dependent portability warnings.
    DebugSeverityError   = 3, // Used for critical errors, API abuses, or undefined behaviors that will likely result in rendering corruption or application crashes.
    DebugSeverityCount,
} DebugSeverity;
